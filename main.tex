...
User guidance is given in the form of an intermediate configuration $\hat{q}$ which, in turn, is used to bias the progress of the planner towards $\hat{q}$.
We demonstrate our approach for the case where the planning algorithm is Multi-Heuristic A* (MHA*) and the robot is a \os{XXX}-DOF humanoid.
\end{abstract}
...
$s(i)$ be the node expanded from~$\Q$ at the $i$th iteration and $\omega$ a parameter.
We define $\kappa_\Q(\omega,i) = \min \{h_\Q(s(i-\omega)), \ldots h_\Q(s(i))\}$ to be the minimum heuristic value attained by the last $\omega$ nodes that were expanded from~$\Q$ (assuming we are at the $i$'th iteration).
...
\label{sec:q3}
To provide completeness guaranteesm, MHA* has to have at least one admissible heuristic~$h_{0}$.
...
cost to reach the goal (via the term~$h_0$) by passing through $\hat{q}$ (via the term $h_{\hat{q}}$). If the state was reached by passing through $\hat{q}$, then the value of $\hat{h}$ is simply the estimation of the cost to reach the goal.
...
As we can see \os{detail experiment + add figure}
