...
as well as in the supplementary videos\footnote{Planning with guidance
\url{https://drive.google.com/open?id=0B5cCvECYZLYZTjFZeGR0WG40QlU}.}
\footnote{Planning without guidance \url{https://drive.google.com/open?id=0B5cCvECYZLYZdFM2SU1CUnJzNU0}.
}. 


\begin{figure}[tb]
  \centering
  	\includegraphics[width=0.45\textwidth]{heuristic_value.jpg}
  	\vspace{-2mm}
  \caption{
		...
		Plots depicting the baseline heuristic values when only a limited amount of guidance is given demonstrate that without guidance, the planner remains stuck in the local minima.	
		}
\vspace{-5mm}
   	\label{fig:res}
\end{figure}
